...
pipeline{
    ...
    stages {
        ...
        stage("TRIVY"){
            steps{
                sh "trivy image kalpeshmohanta/netflix:latest > trivyimage.txt" 
            }
        }
        stage('Deploy to container'){
            steps{
                sh 'docker run -d --name netflix -p 8081:80 kalpeshmohanta/netflix:latest'
            }
        }
    }
}
...
- `Windows: Winget`
- Install `Helm` from [Official Doc](https://helm.sh/docs/intro/install/) 
